...
export default class ArcheType {
    ...
    constructor(world: World, componentDefinitions: Array<any> | Uint32Array, mask: Uint32Array) {
    ...
    transferEntity(entityId: number, oldArcheType: ArcheType) {
        ...
        for (let index = 0; index < otherComponentDefinitions.length(); index++) {
            ...
            if (!this.columnDefinitions.has(transferSparseId)) {
                ...
                this.clearEntityColumn(columnToTransfer, oldEntity);
                continue;
            }
    ...
    hasComponents(components: Uint32Array | Array<number>) {
    ...
    hasNotComponents(components: Uint32Array | Array<number>) {
